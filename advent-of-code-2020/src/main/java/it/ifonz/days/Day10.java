...
public class Day10 {
	...
	public static void part2(List<Integer> numbers) {
		...
		System.out.println(pathsBetween(numbers, 0, device, cache));
	}
	
	private static long pathsBetween(List<Integer> numbers, int source, int destination, HashMap<Integer, Long> cache) {
		...
		for (int i = 1; i <= 3; i++) {
			if (numbers.contains(source+i))
				paths += pathsBetween(numbers, source+i, destination, cache);
		}
